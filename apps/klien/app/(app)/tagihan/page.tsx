...
const REKENING_PERUSAHAAN = {
  bank: 'BCA',
  nomor: '1234567890',
  atas_nama: 'PT Mikala Global Medika',
};
...
export default function TagihanPage() {
  ...
  const handleXenditPay = async () => {
    ...
    try {
      ...
      alert('Gagal membuat link pembayaran. Coba lagi atau hubungi admin.');
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:'12px' }}>
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5 text-xs" style={{ color:'var(--text-muted)' }}>
                          <Calendar size={11} />
                          <span>Jatuh Tempo: {new Date(invoice.due_date || invoice.tanggal_jatuh_tempo).toLocaleDateString('id-ID')}</span>
                        </div>
                      </div>
                    </div>
              ...
              <>
                ...
                {paymentMethod === '' && (
                  <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
                    <button onClick={() => setPaymentMethod('xendit')}
                      style={{ display:'flex', alignItems:'center', gap:'14px', padding:'16px', background:'var(--glass)', border:'2px solid var(--border)', borderRadius:'16px', cursor:'pointer', textAlign:'left' }}>
                      ...
                      <div>
                        ...
                        <p style={{ color:'var(--text3)', fontSize:'12px', marginTop:'2px' }}>Kartu kredit, transfer virtual account, QRIS, e-wallet</p>
                      </div>
                    </button>
                    ...
                    <button onClick={() => setPaymentMethod('manual')}
                      ...
                      <div>
                        ...
                        <p style={{ color:'var(--text3)', fontSize:'12px', marginTop:'2px' }}>Transfer ke rekening perusahaan lalu konfirmasi</p>
                      </div>
                    </button>
                  </div>
                )}
                ...
                {paymentMethod === 'xendit' && (
                  <div style={{ display:'flex', flexDirection:'column', gap:'14px' }}>
                    <button onClick={() => setPaymentMethod('' )} style={{ display:'flex', alignItems:'center', gap:'6px', background:'none', border:'none', color:'var(--text2)', fontSize:'13px', cursor:'pointer', padding:0 }}>
                      ← Kembali
                    </button>
                    <div style={{ background:'rgba(79,70,229,0.08)', border:'1px solid rgba(79,70,229,0.2)', borderRadius:'14px', padding:'16px', display:'flex', flexDirection:'column', gap:'8px' }}>
                      <p style={{ fontWeight:600, fontSize:'13px', color:'var(--text)' }}>Pembayaran via Xendit</p>
                      <p style={{ color:'var(--text3)', fontSize:'12px', lineHeight:'1.6' }}>Anda akan diarahkan ke halaman pembayaran Xendit. Tersedia metode: Kartu Kredit/Debit, Virtual Account, QRIS, GoPay, OVO, Dana, dan lainnya.</p>
                    </div>
                ...
                {paymentMethod === 'manual' && (
                  <div style={{ display:'flex', flexDirection:'column', gap:'14px' }}>
                    ...
                    <div style={{ background:'rgba(14,165,233,0.08)', border:'1px solid rgba(14,165,233,0.2)', borderRadius:'14px', padding:'16px', display:'flex', flexDirection:'column', gap:'12px' }}>
                      ...
                      {[
                        { label:'Bank', value: REKENING_PERUSAHAAN.bank },
                        { label:'Nomor Rekening', value: REKENING_PERUSAHAAN.nomor, copyable: true },
                        { label:'Atas Nama', value: REKENING_PERUSAHAAN.atas_nama },
