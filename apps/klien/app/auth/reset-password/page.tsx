        ...
        <div style={{ background:'rgba(255,255,255,0.05)', backdropFilter:'blur(20px)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:'24px', padding:'28px' }}>
          ...
          {validating && (
            <div style={{ textAlign:'center', padding:'20px' }}>
              <div style={{ width:'40px', height:'40px', borderRadius:'50%', border:'3px solid rgba(124,58,237,0.3)', borderTopColor:'#10b981', animation:'spin 0.8s linear infinite', margin:'0 auto 12px' }}/>
          ...
          {!validating && !tokenValid && !success && (
            <div style={{ textAlign:'center' }}>
              ...
              <button onClick={() => router.push('/auth/forgot-password')}
                style={{ width:'100%', padding:'13px', background:'linear-gradient(135deg,#10b981,#059669)', border:'none', borderRadius:'14px', color:'white', fontSize:'14px', fontWeight:700, cursor:'pointer' }}>
                Request Reset Ulang
              </button>
            </div>
          )}
          ...
          {!validating && tokenValid && !success && (
            <>
              ...
              <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:'14px' }}>
                <div>
                  <label style={{ color:'rgba(16,185,129,0.8)', fontSize:'13px', fontWeight:500, display:'block', marginBottom:'8px' }}>Password Baru *</label>
                  <div style={{ position:'relative' }}>
                    <input type={showPass ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)}
                      placeholder="Min. 8 karakter" required style={inp}/>
                    <button type="button" onClick={() => setShowPass(!showPass)} style={{ position:'absolute', right:'14px', top:'50%', transform:'translateY(-50%)', background:'none', border:'none', color:'rgba(16,185,129,0.6)', cursor:'pointer', display:'flex' }}>
                      {showPass ? <EyeOff size={16}/> : <Eye size={16}/>}
                    </button>
                  </div>
                </div>
                <div>
                  <label style={{ color:'rgba(16,185,129,0.8)', fontSize:'13px', fontWeight:500, display:'block', marginBottom:'8px' }}>Konfirmasi Password *</label>
                ...
                <button type="submit" disabled={loading} style={{
                  ...
                  background: loading ? 'rgba(124,58,237,0.5)' : 'linear-gradient(135deg,#10b981,#059669)',
...
export default function ResetPasswordPage() {
  return (
    <Suspense fallback={
      <div style={{ minHeight:'100vh', background:'linear-gradient(135deg,#0a1f1a,#0d2818,#0a1f1a)', display:'flex', alignItems:'center', justifyContent:'center' }}>
