...
import { DollarSign, Search, Plus, ChevronRight } from 'lucide-react';
...
export default function Page() {
  ...
  useEffect(() => {
    apiClient.get('/internal/finance/invoices')
      .then((res: any) => {
  ...
  return (
    <div className="space-y-5">
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'12px' }}>
        <div>
          <h1 style={{ fontSize:'22px', fontWeight:700, color:'var(--text)' }}>Finance</h1>
          <p style={{ color:'var(--text3)', fontSize:'13px', marginTop:'2px' }}>{data.length} total data</p>
        </div>
        <button style={{ display:'flex', alignItems:'center', gap:'6px', padding:'9px 16px', background:'linear-gradient(135deg, #f59e0b, #d97706)', border:'none', borderRadius:'12px', color:'white', fontWeight:600, fontSize:'13px', cursor:'pointer', boxShadow:'0 4px 12px rgba(245,158,11,0.3)' }}>
          <Plus size={15} />Tambah
        </button>
      </div>
        ...
        {loading ? (
          ...
          <div style={{ textAlign:'center', padding:'48px 20px' }}>
            <div style={{ width:'64px', height:'64px', borderRadius:'20px', margin:'0 auto 14px', background:'rgba(124,58,237,0.08)', display:'flex', alignItems:'center', justifyContent:'center' }}>
              <DollarSign size={28} style={{ color:'var(--purple-light)', opacity:0.5 }} />
            </div>
            <p style={{ fontWeight:600, color:'var(--text)', marginBottom:'6px' }}>Belum ada data</p>
            <p style={{ color:'var(--text3)', fontSize:'13px' }}>Data Finance akan muncul di sini</p>
          </div>
        )}
      </div>
    </div>
