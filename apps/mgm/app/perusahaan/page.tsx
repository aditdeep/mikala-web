...
export default async function PerusahaanPage() {
  ...
  const lokerUrl = s.prsh_loker_url || MITRA_REGISTER_URL;
  const checklistImage = s.prsh_checklist_image || direkturFoto;
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#eef8fa' }}>
      ...
      {/* ═══ VISI MISI ═══ */}
      <section style={{ padding:'0 20px clamp(48px,8vw,80px)', background:'white', position:'relative', overflow:'hidden' }} className="section-pad">
        <div style={{
          position:'absolute', inset:0, backgroundImage:`url(${LOGO_ROUND})`, backgroundRepeat:'no-repeat',
          backgroundPosition:'center', backgroundSize:'min(50%, 380px)', opacity:0.07, pointerEvents:'none',
        }} />
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(min(100%,260px), 1fr))', gap:'18px' }}>
            {misiList.map((m, i) => (
              <ScrollFade key={i} delay={i*100}>
                <div style={{
                  background: `linear-gradient(150deg, ${GREEN} 0%, #0a6d85 100%)`,
                  color:'white', padding:'26px 24px', borderRadius:'22px', fontSize:'13.5px', lineHeight:1.8,
                  boxShadow:'0 12px 30px rgba(14,146,179,0.2)', height:'100%',
                }}>
                  <div style={{ width:'34px', height:'34px', borderRadius:'10px', background:'rgba(255,255,255,0.2)', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:800, fontSize:'14px', marginBottom:'14px' }}>{i+1}</div>
                  {m}
                </div>
              </ScrollFade>
            ))}
        ...
        <div style={{ maxWidth:'1100px', margin:'0 auto' }}>
          ...
          {legalitasImages.length > 0 && (
            <div style={{ marginBottom:'28px' }}>
              <SecureGallery images={legalitasImages.slice(0,4)} />
            </div>
          )}
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(min(100%,220px), 1fr))', gap:'16px' }}>
            {[
              ...
              <ScrollFade key={c.l} delay={i*100}>
                <div style={{ background:`linear-gradient(135deg, ${GREEN}, ${PINK})`, borderRadius:'16px', padding:'18px 20px', color:'white', textAlign:'center' }}>
        ...
        <div style={{ maxWidth:'1000px', margin:'0 auto', display:'grid', gridTemplateColumns:'1.2fr 1fr', gap:'40px', alignItems:'center' }} className="two-col">
          ...
          <ScrollFade delay={150}>
            <div style={{ borderRadius:'20px', overflow:'hidden', boxShadow:'0 20px 50px rgba(0,0,0,0.25)' }} className="hide-mobile">
              <img src={checklistImage} alt="Mikala" style={{ width:'100%', height:'340px', objectFit:'cover' }} />
            </div>
          </ScrollFade>
        </div>
      </section>
      ...
      <section id="mga" style={{ padding:'clamp(48px,8vw,80px) 20px', background:'#eef8fa' }} className="section-pad">
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', flexDirection:'column', gap:'20px' }}>
          <ScrollFade>
            <div style={{ background:'white', borderRadius:'24px', overflow:'hidden', boxShadow:'0 4px 24px rgba(0,0,0,0.06)', border:`1px solid ${GREEN}10`, display:'grid', gridTemplateColumns:'1fr 1.3fr', gap:'0' }} className="two-col mga-grid">
              {mgaImages.length > 0 && (
                <div className="hide-mobile mga-image-wrap" style={{ position:'relative', minHeight:'260px', clipPath:'polygon(0 0, 100% 0, 88% 100%, 0 100%)' }}>
                  <ImageFade images={mgaImages} alt="Mikala Global Akademi" arrows />
                </div>
              )}
              <div style={{ padding:'clamp(24px,4vw,36px)', position:'relative', marginLeft: mgaImages.length > 0 ? '-24px' : 0 }} className="mga-text-panel">
      ...
      <style>{`
        .galeri-marquee-content { display:flex; gap:16px; width:max-content; animation: mgm-marquee 30s linear infinite; }
        .galeri-marquee-track:hover .galeri-marquee-content { animation-play-state: paused; }
        ...
        @media (max-width: 768px) {
          ...
          .mga-grid { grid-template-columns: 1fr !important; }
          .mga-image-wrap { clip-path: none !important; }
          .mga-text-panel { margin-left: 0 !important; }
        }
      `}</style>
    </div>
  );
